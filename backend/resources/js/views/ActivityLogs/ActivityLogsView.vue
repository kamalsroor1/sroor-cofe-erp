    ...
    <div class="space-y-6 max-w-7xl mx-auto font-tajawal">
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-950/80 p-5 rounded-2xl border border-slate-800 shadow-xl">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-xl font-black text-white">سجل التدقيق الأمني والنشاطات</h1>
            <p class="text-xs text-slate-400">تتبع كافة العمليات وحركات النظام، الدخول، الإلغاءات، والتعديلات الحساسة</p>
          </div>
        </div>
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 shadow-lg space-y-1">
          <div class="text-slate-400 text-xs font-bold">نشاطات اليوم</div>
          <div class="text-2xl font-black text-white font-mono">{{ stats.today_total || 0 }}</div>
          <div class="text-[10px] text-slate-500">سجل مسجل اليوم</div>
        </div>

        <div class="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 shadow-lg space-y-1">
          <div class="text-rose-400 text-xs font-bold">عمليات حساسة / حرجة</div>
          <div class="text-2xl font-black text-rose-400 font-mono">{{ stats.today_critical || 0 }}</div>
          <div class="text-[10px] text-slate-500">حذف / إلغاء فواتير / فشل دخول</div>
        </div>

        <div class="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 shadow-lg space-y-1">
          <div class="text-amber-400 text-xs font-bold">الموظفون المتفاعلون</div>
          <div class="text-2xl font-black text-amber-400 font-mono">{{ stats.today_users || 0 }}</div>
          <div class="text-[10px] text-slate-500">أجروا عمليات اليوم</div>
        </div>

        <div class="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 shadow-lg space-y-1">
          <div class="text-cyan-400 text-xs font-bold">الفروع النشطة</div>
          <div class="text-2xl font-black text-cyan-400 font-mono">{{ stats.today_stores || 0 }}</div>
          <div class="text-[10px] text-slate-500">سجلت حركات اليوم</div>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div class="relative">
            ...
            <input
              ...
              type="text"
              placeholder="بحث بالوصف، IP، الموظف..."
      ...
      <div class="bg-slate-950/80 rounded-2xl border border-slate-800 shadow-xl overflow-hidden">
        <div v-if="isLoading" class="p-16 text-center">
          ...
          <p class="text-xs text-slate-400">جاري تحميل سجل النشاطات...</p>
        </div>

        <div v-else-if="logs.length === 0" class="p-16 text-center">
          <Activity class="w-12 h-12 text-slate-600 mx-auto mb-3" />
          <h3 class="text-sm font-bold text-slate-300 mb-1">لا توجد سجلات نشاط مطابقة</h3>
          <p class="text-xs text-slate-500">جرب تعديل خيارات الفلترة والبحث.</p>
        </div>
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="space-y-1">
                ...
                <div class="flex items-center gap-3 text-[11px] text-slate-400 flex-wrap font-sans">
                  <span>الموظف: <strong class="text-slate-300">{{ log.user_name }}</strong></span>
                  <span>الفرع: <strong class="text-slate-300">{{ log.store_name }}</strong></span>
                  <span v-if="log.ip_address" class="font-mono text-slate-500">IP: {{ log.ip_address }}</span>
                </div>
              </div>
            </div>
        ...
        <div class="bg-slate-950 border border-slate-800 rounded-3xl max-w-xl w-full p-6 shadow-2xl space-y-4">
          <div class="flex items-center justify-between border-b border-slate-800 pb-3">
            <h2 class="text-base font-black text-white flex items-center gap-2">
              ...
              <span>تفاصيل السجل رقم #{{ selectedLog.id }}</span>
            </h2>
          ...
          <div class="space-y-3 text-xs">
            <div class="p-3 bg-slate-900/60 rounded-xl border border-slate-800">
              <div class="text-slate-400 font-bold mb-1">الوصف:</div>
