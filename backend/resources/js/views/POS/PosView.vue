    ...
    <header class="h-14 bg-slate-900 border-b border-slate-800 px-4 flex items-center justify-between shrink-0">
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex items-center gap-2">
          ...
          <div>
            <h1 class="text-sm font-black text-white leading-none">نقطة البيع السريعة (POS)</h1>
            <span class="text-[10px] text-slate-400 font-bold font-mono">{{ activeStore?.name || 'الفرع الرئيسي' }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="hidden sm:flex items-center gap-2 bg-slate-950 border border-slate-800 px-3 py-1 rounded-xl text-xs">
        ...
        <span v-if="activeShift" class="font-bold text-slate-300">
          الوردية: <span class="font-mono text-amber-400">{{ activeShift.shift_number }}</span>
        </span>
        <span v-else class="text-rose-400 font-bold">
          تنبيه: لا توجد وردية مفتوحة
        </span>
      </div>
    ...
    <div class="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-0 overflow-hidden">
      ...
      <div class="lg:col-span-7 flex flex-col border-b lg:border-b-0 lg:border-e border-slate-800 bg-slate-900/40 p-4 space-y-3 overflow-y-auto">
        ...
        <div class="flex items-center gap-2">
          <div class="relative flex-1">
            <input
              ...
              placeholder="بحث بالاسم، الكود، أو مسح الباركود (F2)..."
              autofocus
            >
        ...
        <div v-if="isLoading" class="p-12 text-center">
          ...
          <p class="text-xs text-slate-400 font-bold">جاري تحميل الأصناف...</p>
        </div>
          ...
          >
            ...
            <div class="flex items-center justify-between pt-1 border-t border-slate-800/60">
              <span class="text-sm font-black text-amber-400 font-mono">
                {{ formatMoney(getItemPrice(item)) }} <span class="text-[10px] text-slate-400 font-normal">ج.م</span>
              </span>
      ...
      <div class="lg:col-span-5 flex flex-col bg-slate-950 p-4 space-y-3 justify-between h-full">
        ...
        <div class="space-y-2 pb-2 border-b border-slate-800">
          <div class="flex items-center gap-2">
            ...
            >
              {{ activePriceTier === 'wholesale' ? '📦 جملة' : '🛍️ قطاعي' }}
            </button>
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto space-y-2 max-h-[calc(100vh-420px)] pr-1">
          <div v-if="cart.length === 0" class="h-48 flex flex-col items-center justify-center text-slate-600 text-xs space-y-2">
            ...
            <span>السلة فارغة، اضغط على صنف للإضافة</span>
          </div>
          ...
          >
            <div class="flex-1 min-w-0">
              ...
              <div class="text-[10px] text-slate-400 font-mono">
                {{ formatMoney(item.unit_price) }} ج.م × {{ item.quantity }} = <span class="font-bold text-amber-400">{{ formatMoney(item.quantity * item.unit_price) }} ج.م</span>
              </div>
            </div>
        ...
        <div class="space-y-3 pt-2 border-t border-slate-800 bg-slate-950">
          ...
          <div class="space-y-1.5 font-mono text-xs">
            <div class="flex justify-between text-slate-400 font-sans">
              <span>الإجمالي:</span>
              <span class="font-mono text-white">{{ formatMoney(cartSubtotal) }} ج.م</span>
            </div>

            <div class="flex items-center justify-between gap-2">
              <span class="text-slate-400 font-sans text-xs">الخصم:</span>
            ...
            <div class="flex justify-between text-sm font-black text-white pt-1 border-t border-slate-800 font-sans">
              ...
              <span class="font-mono text-emerald-400 text-base">{{ formatMoney(cartNetTotal) }} ج.م</span>
            </div>
          </div>
          ...
          <div v-if="paymentType === 'partial'" class="flex items-center justify-between gap-2">
            <span class="text-xs font-bold text-slate-400 font-sans">المدفوع نقداً الآن:</span>
            <input
              v-model="paidAmount"
              type="number"
              step="0.001"
          ...
          >
            ...
            <span>حفظ واعتماد الفاتورة (F9) ⚡</span>
          </button>
        </div>
      </div>
    </div>
      ...
      <form @submit.prevent="submitQuickCustomer" class="space-y-4 font-tajawal">
        <div>
          ...
          <input
            ...
            autofocus
            class="w-full h-10 px-3 bg-slate-900 border border-slate-700 rounded-xl text-xs text-white focus:ring-2 focus:ring-amber-500 focus:outline-none"
            placeholder="الاسم الكامل أو اسم الكافيه..."
          >
        </div>
        ...
        <div>
          ...
          <select
            ...
            class="w-full h-10 px-3 bg-slate-900 border border-slate-700 rounded-xl text-xs text-white focus:ring-2 focus:ring-amber-500 focus:outline-none"
          >
            <option value="retail">🛍️ قطاعي</option>
    ...
    >
      <div v-if="lastCreatedInvoice" class="space-y-4 font-tajawal text-center">
        ...
        <div>
          ...
          <div class="text-xs text-slate-400 mt-0.5">العميل: {{ lastCreatedInvoice.customer_name }}</div>
        </div>

        <div class="p-3.5 bg-slate-900 border border-slate-800 rounded-2xl space-y-1 font-mono text-xs">
          <div class="flex justify-between text-slate-300">
            <span>صافي الفاتورة:</span>
            <span class="font-black text-emerald-400">{{ formatMoney(lastCreatedInvoice.net_total) }} ج.م</span>
          </div>
          <div class="flex justify-between text-slate-400">
            <span>المدفوع:</span>
            <span>{{ formatMoney(lastCreatedInvoice.paid_amount) }} ج.م</span>
          </div>
          <div v-if="lastCreatedInvoice.remaining_amount > 0" class="flex justify-between text-rose-400 font-bold">
            <span>المتبقي آجل:</span>
            <span>{{ formatMoney(lastCreatedInvoice.remaining_amount) }} ج.م</span>
          </div>
        </div>
...
const submitQuickCustomer = async () => {
    try {
        ...
        Swal.fire({ icon: 'error', title: 'خطأ', text: error.userMessage || 'تعذر إضافة العميل' });
    }
};
...
const submitCheckout = async () => {
    ...
    try {
        ...
        Swal.fire({
            ...
            text: error.userMessage || 'تعذر حفظ واعتماد فاتورة البيع',
        });
    } finally {
