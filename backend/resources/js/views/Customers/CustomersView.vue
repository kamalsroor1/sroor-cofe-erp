      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="p-5 rounded-2xl bg-slate-950/80 border border-slate-800 shadow-lg space-y-2">
          ...
          <div class="text-2xl font-black text-rose-400 font-mono">
            {{ formatMoney(metrics.total_debt || 0) }} <span class="text-xs text-slate-400">ج.م</span>
          </div>
        ...
        <div class="p-5 rounded-2xl bg-slate-950/80 border border-slate-800 shadow-lg space-y-2">
          ...
          <div class="text-2xl font-black text-amber-400 font-mono">
            {{ metrics.debtors_count || 0 }} <span class="text-xs text-slate-400">عميل</span>
          </div>
          <div class="text-[11px] text-slate-500 font-tajawal">
            عملاء عليهم مديونيات بحاجة للتحصيل
          </div>
        </div>
        ...
        <div class="p-5 rounded-2xl bg-slate-950/80 border border-slate-800 shadow-lg space-y-2">
          ...
          <div class="text-2xl font-black text-white font-mono">
            {{ metrics.total_customers || 0 }} <span class="text-xs text-slate-400">عميل</span>
          </div>
          <div class="text-[11px] text-slate-500 font-tajawal">
            إجمالي الحسابات المسجلة بالنظام
          </div>
        </div>
      </div>
      ...
      <div class="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 shadow-md flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-1 bg-slate-900 p-1 rounded-xl border border-slate-800 overflow-x-auto">
          ...
          <button
            ...
            class="px-3 py-1.5 rounded-lg text-xs font-bold font-tajawal transition-all whitespace-nowrap cursor-pointer"
            :class="debtStatus === 'debtor' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' : 'text-slate-400 hover:text-slate-200'"
          >
          ...
          <button
            ...
            class="px-3 py-1.5 rounded-lg text-xs font-bold font-tajawal transition-all whitespace-nowrap cursor-pointer"
            :class="debtStatus === 'creditor' ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'text-slate-400 hover:text-slate-200'"
          >
      ...
      <div class="bg-slate-950/80 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
            ...
            <tbody class="divide-y divide-slate-800/60 font-sans">
              ...
              >
                ...
                <td class="py-3.5 px-4">
                  <div class="font-bold text-white font-tajawal text-sm">{{ customer.name }}</div>
                ...
                <td class="py-3.5 px-4 text-end">
                  ...
                  >
                    {{ formatMoney(customer.current_balance) }} <span class="text-xs font-normal">ج.م</span>
                  </div>
                  <div class="text-[10px] font-tajawal text-slate-500 mt-0.5">
                    {{ customer.current_balance > 0 ? 'مدين (مستحق)' : (customer.current_balance < 0 ? 'دائن (رصيد)' : 'مسدد') }}
                  </div>
                </td>
        ...
        <div v-if="pagination.last_page > 1" class="p-4 border-t border-slate-800 flex items-center justify-between">
          <div class="text-xs text-slate-400 font-tajawal">
            إجمالي النتائج: <span class="font-mono text-amber-400">{{ pagination.total }}</span> عميل
          </div>
        ...
        <form @submit.prevent="saveCustomer" class="space-y-4">
          ...
          <div>
            ...
            <textarea
              v-model="form.notes"
              rows="2"
              class="w-full p-2.5 bg-slate-900 border border-slate-700 rounded-xl text-xs text-white focus:ring-2 focus:ring-amber-500 focus:outline-none font-tajawal"
        ...
        <form @submit.prevent="savePayment" class="space-y-4">
          ...
          <div class="p-3.5 bg-slate-900/90 border border-slate-800 rounded-2xl flex items-center justify-between">
            ...
            <span class="text-base font-black font-mono" :class="targetCustomer?.current_balance > 0 ? 'text-rose-400' : 'text-emerald-400'">
              {{ formatMoney(targetCustomer?.current_balance || 0) }} ج.م
            </span>
          </div>
          ...
          <div>
            ...
            <input
              ...
              required
              autofocus
              class="w-full h-11 px-3 bg-slate-900 border border-slate-700 rounded-xl text-base font-bold text-emerald-400 font-mono focus:ring-2 focus:ring-emerald-500 focus:outline-none"
              placeholder="0.00"
            >
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                required
                class="w-full h-10 px-3 bg-slate-900 border border-slate-700 rounded-xl text-xs text-white focus:ring-2 focus:ring-emerald-500 focus:outline-none font-tajawal"
              >
            ...
            <div>
              ...
              <input
                ...
                type="date"
                required
                class="w-full h-10 px-3 bg-slate-900 border border-slate-700 rounded-xl text-xs text-white font-mono focus:ring-2 focus:ring-emerald-500 focus:outline-none"
              >
            </div>
          </div>
...
const saveCustomer = async () => {
    ...
    try {
        ...
        Swal.fire({
            icon: 'error',
            title: 'خطأ',
            text: error.userMessage || 'تعذر حفظ بيانات العميل',
        });
    } finally {
...
const openPaymentModal = (c) => {
    ...
    paymentForm.notes = 'سند قبض وتحصيل نقدي';
...
const savePayment = async () => {
    ...
    try {
        ...
        Swal.fire({
            icon: 'success',
            title: 'تم التحصيل',
            text: 'تم تسجيل سند القبض بنجاح وتحديث رصيد العميل',
            timer: 1500,
            showConfirmButton: false,
        });
    ...
    if (result.isConfirmed) {
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'خطأ',
                text: error.userMessage || 'تعذر حذف العميل',
            });
        }
    }
};
