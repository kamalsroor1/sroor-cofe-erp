<template>
  <div class="space-y-6 max-w-7xl mx-auto font-tajawal">
      ...
      <div class="p-4 rounded-2xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 shadow-md flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        <!-- Search Input -->
        <BaseSearchInput v-model="searchQuery" placeholder="$t('purchases.search_purchases_placeholder')" :debounce="300" />
        ...
        <div v-if="selectedPurchase" class="space-y-4 font-tajawal text-xs">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5 p-3.5 bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl">
            ...
            <div>
              ...
              <span class="text-slate-900 dark:text-slate-200 font-mono">{{ selectedPurchase.purchase_date }}</span>
            </div>
            <div>
              <span class="text-slate-400 block font-bold">{{ $t('purchases.received_branch') }}:</span>
              <span class="text-slate-900 dark:text-slate-200">{{ selectedPurchase.store_name || $t('common.main_branch') }}</span>
            </div>
            <div>
