<template>
  <div class="space-y-6 max-w-7xl mx-auto font-tajawal">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="p-5 rounded-2xl bg-slate-950/80 border border-slate-800 shadow-lg space-y-2">
          ...
          <div class="text-2xl font-black text-emerald-400 font-mono">
            {{ formatMoney(metrics.total_purchases || 0) }} <span class="text-xs text-slate-400">ج.م</span>
          </div>
          <div class="text-[11px] text-slate-500">
            إجمالي قيمة المشتريات وتوريد الخامات المعتمدة
          </div>
        </div>
        ...
        <div class="p-5 rounded-2xl bg-slate-950/80 border border-slate-800 shadow-lg space-y-2">
          ...
          <div class="text-2xl font-black text-rose-400 font-mono">
            {{ formatMoney(metrics.unpaid_total || 0) }} <span class="text-xs text-slate-400">ج.م</span>
          </div>
          <div class="text-[11px] text-slate-500">
            مستحقات آجلة للموردين قيد السداد
          </div>
        </div>
        ...
        <div class="p-5 rounded-2xl bg-slate-950/80 border border-slate-800 shadow-lg space-y-2">
          ...
          <div class="text-2xl font-black text-white font-mono">
            {{ metrics.confirmed_count || 0 }} <span class="text-xs text-slate-400">فاتورة</span>
          </div>
          <div class="text-[11px] text-slate-500">
            عدد فواتير التوريد المنفذة بالمخازن
          </div>
        </div>
      </div>
      ...
      <div class="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 shadow-md flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        ...
        <div class="relative flex-1">
          <input
            ...
            placeholder="بحث برقم الفاتورة، اسم المورد، أو رقم إذن التوريد..."
          >
        ...
        <div class="w-full md:w-44">
          ...
          >
            ...
            <option value="confirmed">معتمدة وموردة ✅</option>
            <option value="cancelled">ملغاة ومسترجعة 🚫</option>
          </select>
        </div>
            ...
            <tbody class="divide-y divide-slate-800/60 font-sans">
              ...
              >
                ...
                <td class="py-3.5 px-4 text-center font-tajawal">
                  ...
                  >
                    {{ p.status === 'confirmed' ? 'معتمدة' : 'ملغاة' }}
                  </span>
                </td>
      ...
      <AppModal
        ...
        :title="`تفاصيل فاتورة المشتريات: ${selectedPurchase?.purchase_number}`"
      ...
      >
        <div v-if="selectedPurchase" class="space-y-4 font-tajawal text-xs">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5 p-3.5 bg-slate-900/80 border border-slate-800 rounded-2xl">
            <div>
              <span class="text-slate-400 block font-bold">المورد:</span>
              <span class="text-white font-bold">{{ selectedPurchase.supplier_name }}</span>
            </div>
            <div>
              <span class="text-slate-400 block font-bold">تاريخ الفاتورة:</span>
              <span class="text-slate-200 font-mono">{{ selectedPurchase.purchase_date }}</span>
            </div>
            <div>
              <span class="text-slate-400 block font-bold">الفرع المستلم:</span>
              <span class="text-slate-200">{{ selectedPurchase.store_name || 'المخزن الرئيسي' }}</span>
            </div>
            <div>
              <span class="text-slate-400 block font-bold">الحالة:</span>
              <span class="font-bold" :class="selectedPurchase.status === 'confirmed' ? 'text-emerald-400' : 'text-rose-400'">
                {{ selectedPurchase.status === 'confirmed' ? 'معتمدة وموردة' : 'ملغاة' }}
              </span>
            </div>
          </div>
            ...
            <table class="w-full text-start text-xs border-collapse">
              <thead>
                <tr class="bg-slate-900 text-slate-400 border-b border-slate-800">
                  <th class="p-2.5 text-start font-bold">الصنف / الخامة</th>
                  <th class="p-2.5 text-end font-bold">الكمية</th>
                  <th class="p-2.5 text-end font-bold">سعر الشراء</th>
              ...
              <tbody class="divide-y divide-slate-800/50">
                <tr v-for="item in selectedPurchase.items" :key="item.id">
                  ...
                  <td class="p-2.5 text-end font-mono text-slate-300">{{ formatMoney(item.cost_price) }} ج.م</td>
                  <td class="p-2.5 text-end font-mono font-bold text-emerald-400">{{ formatMoney(item.total_price) }} ج.م</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="p-3 bg-slate-900 border border-slate-800 rounded-xl space-y-1.5 font-mono text-xs">
            <div class="flex justify-between text-slate-300">
              ...
              <span>{{ formatMoney(selectedPurchase.subtotal) }} ج.م</span>
            </div>
            <div v-if="selectedPurchase.discount_amount > 0" class="flex justify-between text-rose-400">
              <span>الخصم المكتسب:</span>
              <span>-{{ formatMoney(selectedPurchase.discount_amount) }} ج.م</span>
            </div>
            <div v-if="selectedPurchase.additional_expenses_total > 0" class="flex justify-between text-amber-400">
              <span>مصاريف محملة ملحقة:</span>
              <span>+{{ formatMoney(selectedPurchase.additional_expenses_total) }} ج.م</span>
            </div>
            <div class="flex justify-between text-base font-black text-white pt-2 border-t border-slate-800">
              <span>صافي الفاتورة:</span>
              <span class="text-emerald-400">{{ formatMoney(selectedPurchase.net_total) }} ج.م</span>
            </div>
          </div>
        </div>
      </AppModal>
...
const cancelPurchase = async (p) => {
    const result = await Swal.fire({
        title: `إلغاء فاتورة الشراء (${p.purchase_number})؟`,
        text: 'سيتم خصم الكميات من المخزون وعكس مديونية المورد وسندات الصرف.',
        icon: 'warning',
    ...
    if (result.isConfirmed) {
        try {
            await api.post(`/purchases/${p.id}/cancel`, { reason: 'إلغاء من واجهة النظام' });
            Swal.fire({
                icon: 'success',
                title: 'تم الإلغاء',
                text: 'تم إلغاء فاتورة المشتريات وعكس المخزون بنجاح',
                timer: 1500,
            ...
            Swal.fire({
                icon: 'error',
                title: 'خطأ',
                text: error.userMessage || 'تعذر إلغاء فاتورة الشراء',
            });
        }
    }
};
