  ...
  <aside class="h-full flex flex-col bg-white dark:bg-slate-900/95 border-s border-slate-200 dark:border-slate-800 overflow-hidden w-14 sm:w-36 md:w-40 shrink-0">
    ...
    <nav class="flex-1 overflow-y-auto custom-scrollbar p-1.5 space-y-1" dir="rtl">
      ...
      >
        ...
        <span class="badge bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-400 px-1.5 py-0.2 rounded text-[10px] font-mono hidden sm:inline-block">{{ favoriteCount }}</span>
      </button>
      ...
      >
        ...
        <span class="badge bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-300 px-1.5 py-0.2 rounded text-[10px] font-mono hidden sm:inline-block">{{ cat.items_count }}</span>
      </button>
    </nav>
