  ...
  >
    <form @submit.prevent="$emit('save')" class="space-y-4 font-tajawal">
      ...
      <div class="p-3.5 bg-slate-50 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-2xl flex items-center justify-between">
        <span class="text-xs font-bold text-slate-500 dark:text-slate-400">{{ $t('contacts.payable_balance_label') }}:</span>
        <span class="text-base font-black font-mono" :class="targetSupplier?.current_balance > 0 ? 'text-theme-primary' : 'text-emerald-500'">
      ...
      <div>
        ...
        <textarea
          ...
          :placeholder="$t('contacts.disbursement_notes_placeholder')"
        ></textarea>
      </div>
