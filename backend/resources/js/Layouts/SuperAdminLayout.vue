...
<template>
    <div class="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col antialiased selection:bg-indigo-500 selection:text-white font-tajawal" dir="rtl">
        ...
        <header class="h-16 border-b border-slate-200 dark:border-indigo-900/40 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md sticky top-0 z-40 px-4 flex items-center justify-between shadow-xs">
                ...
                <router-link to="/super-admin/dashboard" class="flex items-center gap-2.5">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            ...
                            <span class="px-2 py-0.5 rounded-full text-[10px] font-black bg-indigo-500/10 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 border border-indigo-500/30 hidden sm:inline-block">
                                {{ $t('super.central_platform') }}
                            </span>
                        </div>
                        <p class="text-[11px] text-slate-500 dark:text-slate-400 font-bold hidden sm:block">
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                ...
                <div class="text-left hidden md:block pl-2 border-r border-slate-200 dark:border-slate-800 pr-3">
                    <div class="text-xs font-black text-slate-900 dark:text-white">{{ user?.name || $t('super.platform_admin') }}</div>
        ...
        <Transition name="sidebar-drawer">
            <aside
                v-if="mobileMenuOpen"
                class="fixed inset-y-0 right-0 z-50 w-72 max-w-[85vw] h-full bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-indigo-900/40 p-4 flex flex-col space-y-6 shadow-2xl md:hidden font-tajawal"
            >
        ...
        <div class="flex-1 flex overflow-hidden">
            ...
            <aside class="hidden md:flex w-64 bg-white dark:bg-slate-900/60 border-l border-slate-200 dark:border-indigo-900/30 flex-col p-4 space-y-6 shrink-0 font-tajawal shadow-xs">
                <div class="space-y-1">
                    <div class="px-3 text-[11px] font-black tracking-wider text-indigo-600 dark:text-indigo-400 uppercase mb-2">
                        {{ $t('super.platform_title') }}
                    </div>
