...
test.describe('ReturnsView Comprehensive 4-Axes Audit & Multi-Viewport Verification', () => {
    ...
    for (const vp of viewports) {
        test(`should render Returns Ledger correctly on ${vp.name}`, async ({ page }) => {
            ...
            await expect(header).toContainText('المرتجعات');

            // 2. Verify New Return Button
            await expect(page.locator('main a[href="/returns/create"]').first()).toBeVisible();

            // 3. Verify Financial Metrics Cards
    ...
    test('should allow searching and filtering returns', async ({ page }) => {
        await page.goto('/returns', { waitUntil: 'networkidle' });
        await page.waitForTimeout(1000);
        await dismissAnyModal(page);
