...
test.describe('PurchasesView Comprehensive 4-Axes Audit & Multi-Viewport Verification', () => {
    ...
    for (const vp of viewports) {
        test(`should render Purchases List correctly on ${vp.name}`, async ({ page }) => {
            ...
            await expect(header).toContainText('المشتريات');

            // Verify New Purchase button in main
            await expect(page.locator('main a[href="/purchases/create"]').first()).toBeVisible();

            // 2. Verify Metrics Grid Cards (Total Purchases, Unpaid, Confirmed count)
            await expect(page.locator('text=إجمالي المشتريات').first()).toBeVisible();
            await expect(page.locator('text=المستحقات غير المسددة').first()).toBeVisible();

            // 3. Verify Search Bar
            const searchInput = page.locator('main input[placeholder*="بحث"]').first();
    ...
    test('should allow searching purchases and opening details modal', async ({ page }) => {
        await page.goto('/purchases', { waitUntil: 'networkidle' });
        await page.waitForTimeout(1000);
        await dismissAnyModal(page);
