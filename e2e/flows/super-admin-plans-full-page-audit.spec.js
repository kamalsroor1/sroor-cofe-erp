...
test.describe('SuperAdminPlansView Comprehensive 4-Axes Audit & Multi-Viewport Verification', () => {
    ...
    for (const vp of viewports) {
        test(`should render Super Admin Plans grid correctly on ${vp.name}`, async ({ page }) => {
            await page.setViewportSize({ width: vp.width, height: vp.height });

            const consoleErrors = [];
            page.on('console', msg => {
                const txt = msg.text();
                if (msg.type() === 'error' && !txt.includes('favicon') && !txt.includes('ERR_CONNECTION_REFUSED')) {
                    consoleErrors.push(txt);
                }
            });

            await page.goto('/super-admin/plans', { waitUntil: 'networkidle' });
            ...
            await expect(header).toContainText('باقات الاشتراك');

            // 2. Verify Plans Cards Rendered
            const planCard = page.locator('main h3').first();
            await expect(planCard).toBeVisible();

            // 3. Verify Edit Buttons Available
            const editBtn = page.locator('button:has-text("تعديل الأسعار")').first();
    ...
    test('should allow opening edit plan modal', async ({ page }) => {
        await page.goto('/super-admin/plans', { waitUntil: 'networkidle' });
        await page.waitForTimeout(1000);
        await dismissAnyModal(page);
        ...
        // Verify Modal Opened
        const modal = page.locator('h3:has-text("تعديل باقة"), h2:has-text("تعديل باقة")').first();
