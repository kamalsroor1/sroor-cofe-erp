...
test.describe('SuperAdminDashboardView Comprehensive 4-Axes Audit & Multi-Viewport Verification', () => {
    ...
    for (const vp of viewports) {
        test(`should render Super Admin Dashboard correctly on ${vp.name}`, async ({ page }) => {
            ...
            await expect(header).toContainText('السوبر أدمن');

            // 2. Verify Action Buttons (Tenants / Plans)
            await expect(page.locator('main a[href="/super-admin/tenants"]').first()).toBeVisible();
            await expect(page.locator('main a[href="/super-admin/plans"]').first()).toBeVisible();
    ...
    test('should verify platform settings inputs exist on super admin dashboard', async ({ page }) => {
        await page.goto('/super-admin', { waitUntil: 'networkidle' });
        await page.waitForTimeout(1000);
        await dismissAnyModal(page);

        // Verify Platform Name input
        const nameInput = page.locator('input[placeholder*="سرور ERP"]').first();
