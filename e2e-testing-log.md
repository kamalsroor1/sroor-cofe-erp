...
## 2. هيكلة حفظ لقطات الشاشة وسياسة التنظيف التلقائي (Screenshot Hierarchy & Auto-Retention)

تُحفظ كافة لقطات الشاشة بنظام زمني هرمي دقيق ومستثنى تماماً من Git، مع تطبيق سياسة تنظيف ذكية تلقائية:
* **رقم التشغيلة المتتابع:** يتم إنشاء مجلد ترقيمي متتابع لكل تشغيلة في اليوم (`run-01`, `run-02`, `run-03`...).
* **الحد الأقصى للتشغيلات اليومية:** بحد أقصى **10 تشغيلات** يومياً (عند بدء التشغيلة 11 يتم حذف أقدم تشغيلة في نفس اليوم تلقائياً).
* **الحد الأقصى لتاريخ الأيام:** الاحتفاظ بآخر **3 أيام فقط** (عند بدء يوم جديد رابع يتم حذف أقدم مجلد يوم بالكامل تلقائياً).

```text
e2e/screenshots/
└── [YYYY-MM-DD]/                      ← مجلد تاريخ اليوم (بحد أقصى آخر 3 أيام)
    └── [run-XX]/                     ← مجلد رقم التشغيلة (بحد أقصى آخر 10 تشغيلات باليوم)
