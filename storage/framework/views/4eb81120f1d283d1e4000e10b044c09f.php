...
<head>
    ...
    ?>
    <title><?php echo e($title ?? $siteCompanyName); ?> | <?php echo e($siteCompanyName); ?> - <?php echo e($siteSubtitle); ?></title>
    ...
    <script>
        (function() {
            try {
                ...
                const userPref = "<?php echo e(auth()->user()->theme_preference ?? ''); ?>";
                ...
                const isPos = <?php echo e(request()->routeIs('invoices.create') ? 'true' : 'false'); ?>;
    ...
    <meta name="apple-mobile-web-app-title" content="<?php echo e($siteCompanyName); ?>">
...
<body class="h-full bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-100 flex overflow-hidden selection:bg-amber-500 selection:text-white" 
      x-data="{ 
          sidebarOpen: false, 
          sidebarCollapsed: <?php echo e(request()->routeIs('invoices.create') ? 'true' : "(localStorage.getItem('sidebar_collapsed') === 'true')"); ?>,
          hasTransition: false,
          toggleSidebar() {
        ...
        >
            ...
            <div class="h-16 px-2.5 flex items-center justify-between border-b border-slate-200 dark:border-slate-800/80 bg-slate-50/70 dark:bg-slate-900/50 backdrop-blur-md overflow-hidden">
                <div class="flex items-center gap-2 min-w-0">
                    <a href="<?php echo e(route('dashboard')); ?>" title="<?php echo e($siteCompanyName); ?>" class="w-9 h-9 rounded-xl bg-white dark:bg-slate-800 p-1 flex items-center justify-center shadow-md shadow-amber-500/10 border border-slate-200 dark:border-slate-700 shrink-0">
                        <img src="<?php echo e(asset('logo.png')); ?>" alt="سرور POS" class="w-full h-full object-contain">
                    </a>
                    <div class="sidebar-text-full truncate min-w-0">
                        <h1 class="font-extrabold text-sm tracking-tight text-slate-900 dark:text-white font-tajawal truncate">
                            <?php echo e($siteCompanyName); ?>

                        </h1>
                        <p class="text-[11px] text-slate-500 dark:text-slate-400 truncate"><?php echo e($siteSubtitle); ?></p>
                    </div>
                </div>
            ...
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('pos.access')): ?>
            ...
            <nav class="flex-1 px-2 py-2.5 space-y-1.5 overflow-y-auto">
                ...
                <div>
                    <a href="<?php echo e(route('dashboard')); ?>" 
                       ...
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('dashboard') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <!-- المبيعات والفواتير -->
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->user()?->can('invoices.view') || auth()->user()?->can('daily_journal.view') || auth()->user()?->can('customers.manage')): ?>
                ...
                <div>
                    <a href="<?php echo e(route('invoices.index')); ?>" 
                       ...
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('invoices.*') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('daily_journal.view')): ?>
                <div>
                    <a href="<?php echo e(route('daily.journal')); ?>" 
                    ...
                    >
                        <div class="relative shrink-0">
                            ...
                            <span class="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full <?php echo e($isOverdueShift ? 'bg-rose-500 animate-ping' : 'bg-emerald-500 animate-pulse'); ?>"></span>
                        ...
                        <div class="sidebar-text-full flex-1 flex items-center justify-between min-w-0">
                            ...
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($activeShift): ?>
                            <span class="text-[10px] px-1.5 py-0.5 rounded-md font-bold <?php echo e($isOverdueShift ? 'bg-rose-500 text-white animate-pulse' : 'bg-emerald-500/20 text-emerald-700 dark:text-emerald-300'); ?>">
                                <?php echo e($isOverdueShift ? 'متأخرة (+24h)' : 'مفتوحة'); ?>
                ...
                <div>
                    <a href="<?php echo e(route('customers.index')); ?>" 
                       ...
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('customers.*') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->user()?->can('items.view') || auth()->user()?->can('transfers.view') || auth()->user()?->can('stores.manage') || auth()->user()?->can('purchases.view') || auth()->user()?->can('suppliers.manage')): ?>
                ...
                <div>
                    <a href="<?php echo e(route('items.index')); ?>" 
                       ...
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('items.*') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <div>
                    <a href="<?php echo e(route('store-stocks')); ?>" 
                       title="جرد وأأسعار الفروع"
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('store-stocks') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <div>
                    <a href="<?php echo e(route('stock-transfers')); ?>" 
                       title="أذونات التحويل والشحن"
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('stock-transfers*') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('stores.manage')): ?>
                <div>
                    <a href="<?php echo e(route('stores')); ?>" 
                       title="الفروع وعربات التوزيع"
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('stores') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <div>
                    <a href="<?php echo e(route('purchases.index')); ?>" 
                       ...
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('purchases.*') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <div>
                    <a href="<?php echo e(route('suppliers.index')); ?>" 
                       ...
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('suppliers.*') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                        <svg class="w-6 h-6 shrink-0 transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->user()?->can('expenses.manage') || auth()->user()?->can('returns.manage') || auth()->user()?->can('reports.view')): ?>
                ...
                <div>
                    <a href="<?php echo e(route('expenses.index')); ?>" 
                       ...
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('expenses.*') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('returns.manage')): ?>
                <div>
                    <a href="<?php echo e(route('returns.index')); ?>" 
                       title="سجل المرتجعات"
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('returns.*') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <div>
                    <a href="<?php echo e(route('reports.index')); ?>" 
                       ...
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('reports.*') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->user()?->can('roles.manage') || auth()->user()?->can('trash.access') || auth()->user()?->can('logs.view')): ?>
                ...
                <div>
                    <a href="<?php echo e(route('users.index')); ?>" 
                       ...
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('users.*') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <div>
                    <a href="<?php echo e(route('roles.index')); ?>" 
                       ...
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('roles.*') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <div>
                    <a href="<?php echo e(route('activity-logs.index')); ?>" 
                       ...
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('activity-logs.*') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <div>
                    <a href="<?php echo e(route('trash.index')); ?>" 
                       title="سلة المحذوفات المركزية"
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('trash.index') ? 'bg-rose-500/15 text-rose-600 dark:text-rose-400 border border-rose-500/40 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-rose-500/10 hover:text-rose-600 dark:hover:text-rose-400'); ?>"
                    >
                ...
                <div>
                    <a href="<?php echo e(route('settings.index')); ?>" 
                       ...
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('settings.index') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
                ...
                <div>
                    <a href="<?php echo e(route('profile')); ?>" 
                       ...
                       class="flex sidebar-nav-item transition-all <?php echo e(request()->routeIs('profile') ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-white'); ?>"
                    >
            ...
            <div class="p-2.5 border-t border-slate-200 dark:border-slate-800/80 bg-slate-50/70 dark:bg-slate-900/60 flex items-center justify-between overflow-hidden">
                <a href="<?php echo e(route('profile')); ?>" title="<?php echo e(auth()->user()->name ?? 'مستخدم'); ?>" class="flex items-center sidebar-user-link min-w-0 hover:opacity-80 transition-opacity">
                    ...
                    <div class="sidebar-user-info min-w-0 truncate">
                        <p class="text-xs font-bold text-slate-800 dark:text-slate-200 truncate"><?php echo e(auth()->user()->name ?? 'مستخدم'); ?></p>
                        <p class="text-[10px] text-amber-600 dark:text-amber-400/80 truncate font-semibold">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->user()->hasRole('admin')): ?> المدير العام
                            <?php elseif(auth()->user()->hasRole('cashier')): ?> كاشير
                            <?php elseif(auth()->user()->hasRole('storekeeper')): ?> أمين مخزن
                            <?php elseif(auth()->user()->hasRole('accountant')): ?> محاسب
                            <?php else: ?> مستخدم <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </p>
                    </div>
                </a>

                <form method="POST" action="<?php echo e(route('logout')); ?>" class="sidebar-logout-btn">
                    <?php echo csrf_field(); ?>
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <header class="h-16 bg-white/90 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 lg:px-8 shrink-0 z-30 shadow-sm">
                ...
                <div class="flex items-center gap-2 sm:gap-3" x-data="{ userMenuOpen: false }">
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->user()?->can('daily_journal.view') || auth()->user()?->hasRole('admin') || auth()->user()?->hasRole('cashier')): ?>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($activeShift): ?>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isOverdueShift): ?>
                            <a href="<?php echo e(route('daily.journal')); ?>" 
                            ...
                            <a href="<?php echo e(route('daily.journal')); ?>" 
                               ...
                               title="اليومية مفتوحة وشغالة حالياً (#<?php echo e($activeShift->shift_number); ?>) - اضغط لمتابعة الدرج أو التقفيل">
                                <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                                <span class="hidden xs:inline">اليومية شغالة</span>
                                <span class="text-[10px] bg-emerald-600/20 text-emerald-800 dark:text-emerald-300 font-mono font-black px-1.5 py-0.5 rounded">#<?php echo e($activeShift->shift_number); ?></span>
                            </a>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        <?php else: ?>
                            <a href="<?php echo e(route('daily.journal')); ?>" 
                    ...
                    <div class="relative" x-data="{ storeMenuOpen: false }">
                        ...
                        >
                            <span>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($currentStore->type === 'wholesale_van'): ?> 🚚 <?php elseif($currentStore->type === 'main_warehouse'): ?> 🏢 <?php else: ?> 🏬 <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </span>
                            <span class="truncate max-w-[100px] sm:max-w-[140px]"><?php echo e($currentStore->name); ?></span>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($availableStores && $availableStores->count() > 1): ?>
                        ...
                        >
                            ...
                            <div class="py-1 max-h-60 overflow-y-auto">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $availableStores; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $stOption): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                                <button
                                    type="button"
                                    onclick="switchGlobalStore(<?php echo e($stOption->id); ?>)"
                                    class="w-full text-right flex items-center justify-between px-3 py-2 text-xs transition-colors cursor-pointer <?php echo e((int)$currentStore->id === (int)$stOption->id ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 font-black' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'); ?>"
                                >
                                    <div class="flex items-center gap-2 truncate">
                                        <span><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($stOption->type === 'wholesale_van'): ?> 🚚 <?php elseif($stOption->type === 'main_warehouse'): ?> 🏢 <?php else: ?> 🏬 <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?></span>
                                        <span class="truncate"><?php echo e($stOption->name); ?></span>
                                    </div>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if((int)$currentStore->id === (int)$stOption->id): ?>
...
$__key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-4108153435-0', $__key);
...
?>
                    ...
                    <div class="relative">
                        ...
                        >
                            ...
                            <span class="font-bold truncate"><?php echo e(auth()->user()->name ?? 'المدير العام'); ?></span>
                        ...
                        >
                            <div class="px-4 py-2 text-xs">
                                <p class="font-bold text-slate-900 dark:text-white"><?php echo e(auth()->user()->name); ?></p>
                                <p class="text-[10px] text-slate-500 dark:text-slate-400 font-mono" dir="ltr"><?php echo e(auth()->user()->phone ?? auth()->user()->email); ?></p>
                            </div>
                            <div class="py-1">
                                <a href="<?php echo e(route('profile')); ?>" class="flex items-center gap-2 px-4 py-2 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-colors">
                                    <span>⚙️</span>
                                    <span>الملف الشخصي والأمان</span>
                                </a>
                                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('roles.manage')): ?>
                                <a href="<?php echo e(route('users.index')); ?>" class="flex items-center gap-2 px-4 py-2 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-colors">
                                    <span>👥</span>
                                    <span>إدارة المستخدمين والصلاحيات</span>
                                </a>
                                <a href="<?php echo e(route('roles.index')); ?>" class="flex items-center gap-2 px-4 py-2 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-colors">
                                    <span>🛡️</span>
                                    <span>مصفوفة الصلاحيات والأدوار</span>
                                </a>
                                <?php endif; ?>
                                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('logs.view')): ?>
                                <a href="<?php echo e(route('activity-logs.index')); ?>" class="flex items-center gap-2 px-4 py-2 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-colors">
                                    <span>📜</span>
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isOverdueShift && (auth()->user()?->hasRole('admin') || auth()->user()?->hasRole('cashier') || auth()->user()?->can('daily_journal.view'))): ?>
            <div class="bg-gradient-to-r from-rose-700 via-rose-600 to-rose-700 text-white px-4 py-2.5 shadow-md flex flex-col sm:flex-row items-center justify-between gap-2 text-xs font-bold shrink-0 z-20 border-b border-rose-800 animate-pulse">
                <div class="flex items-center gap-2 text-center sm:text-right">
                    ...
                    <span>
                        <strong>إنذار تقفيل اليومية:</strong> الوردية رقم (<span class="font-mono underline"><?php echo e($activeShift->shift_number); ?></span>) مفتوحة منذ أكثر من 24 ساعة (منذ <?php echo e($activeShift->opened_at->translatedFormat('l d F - h:i A')); ?>). يرجى مراجعة الخزينة وتقفيل اليومية ومطابقة العهدة!
                    </span>
                </div>
                <a href="<?php echo e(route('daily.journal')); ?>" class="px-3.5 py-1.5 bg-white text-rose-700 hover:bg-rose-100 rounded-xl text-xs font-black transition-all shadow shrink-0 cursor-pointer">
    ...
    <script>
        ...
        window.switchGlobalStore = function(storeId) {
            fetch('<?php echo e(route('store.switch')); ?>', {
                method: 'POST',
                headers: {
    ...
    <div id="pwa-guide-modal" class="fixed inset-0 z-[9999] hidden items-center justify-center p-4 bg-black/80 backdrop-blur-sm" style="display: none;">
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-md p-6 space-y-4 shadow-2xl relative">
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                <div class="flex items-center gap-3">
                    <img src="<?php echo e(asset('logo.png')); ?>" class="w-10 h-10 object-contain rounded-xl p-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700" alt="لوجو">
                    <div>
                        <h3 class="font-black text-slate-900 dark:text-white text-base font-tajawal">تثبيت تطبيق <?php echo e($siteCompanyName); ?></h3>
    ...
    <script>
        ...
        if ('serviceWorker' in navigator) {
            window.addEventListener('load', () => {
                navigator.serviceWorker.register('<?php echo e(asset("sw.js")); ?>').then(reg => {
...
<?php /**PATH I:\projects\erp-2026\backend\resources\views/components/layouts/app.blade.php ENDPATH**/ ?>